...
do
    ...
    do
        ...
        then
            ...
            todo="/usr/bin/time --verbose -o clingo_${filename}.timeout clingo --enum-mode=domRec --heuristic=domain -n 100 --time-limit=3600 clingo_${filename} >> result-${filename}.out 2>&1"
            echo "$todo" >> todo

            # todo="timeout 3600s ./mcsmus -all-cores ${filename} >> result-${filename}.out 2>&1"
            # echo "$todo" >> todo

            todo="/usr/bin/time --verbose -o marco_${filename}.timeout ./marco.py --cnf -T ${tlimit} --threads 1 -v -l 100 ${filename} >> marco-${filename}.out 2>&1"
            echo "$todo" >> todo
